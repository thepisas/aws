...
where :
        ...
        -o arg          set target OU (default \"$TGTOU\")
        -u arg          set username to add host (required)
        -p arg          set password to log in (optional)
"

while getopts "h?vtfd:o:u:p:" opt; do
    case "$opt" in
    h|\?)
        echo "$usage"
        exit 0
...
access_provider = ad
ad_access_filter = (|(memberOf=CN=linux_admin,OU=Groups,OU=Corporate,DC=Amazon,DC=tiffco,DC=net)(memberOf=CN=linux_rstudio_nonprod,OU=Groups,OU=Corporate,DC=Amazon,DC=tiffco,DC=com))
